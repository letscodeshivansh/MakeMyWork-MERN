...
app.post("/login", async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(400).sendFile(path.join(parantdir, "tmeplates", "login.html"), {error: "User Not found"})
        }
        ...
        if (user.floatingPassword === floatingPassword) {
            ...
            res.status(401).sendFile(path.join(parentDir, "templates", "login.html"), { error: "Wrong Password" });
        }
    } catch (error) {
