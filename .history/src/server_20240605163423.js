...
app.use(session({
  // Configure session options (secret key, expiration etc.)
...
app.post("/login", async (req, res) => {
    try {
      ...
      if (user.floatingPassword === floatingPassword) {
        // Login successful, extract username and store in session
        const username = floatingInput.split('@')[0]; // Assuming username in email
  
        // Assuming you have session middleware configured
        req.session.username = username;
  
        // Redirect to postwork with username in query string (optional)
        // res.redirect(`/postwork?username=${username}`);
  
        // Redirect to another page (replace with your desired page)
        res.redirect("/index"); 
      } else {
