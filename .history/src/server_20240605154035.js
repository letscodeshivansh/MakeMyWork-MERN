...
app.get("/postwork", (req, res) => {
    ...
    const username = floatingInput.split('@')[0]; // Assuming you have a username field
        // Login route (assuming successful login)
        res.redirect(`/postwork?username=${loggedInUser.username}`); // Replace loggedInUser with your logic to get username

  
    if (username) {
      // Username available, redirect with username
      return res.redirect(`/postwork?username=${username}`); 
    } else {
      // Username not available
      // Option 1: Redirect to login (assuming you have a login route)
      // return res.redirect('/login'); 
  
      // Option 2: Render the template without username (if applicable)
      res.render("postwork.ejs"); // Replace with your actual template name
    }
});
